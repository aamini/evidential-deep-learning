...
def plot_scatter_with_var(mu, var, path, n_stds=3):
    ...
    for k in np.linspace(0, n_stds, 4):
        plt.fill_between(x_test[:, 0], (mu - k * var)[:, 0], (mu + k * var)[:, 0], alpha=0.3, edgecolor=None,
...
def evidence_reg_2_layers_50_neurons():
    ...
    model, rmse, nll = trainer.train(x_train, y_train, x_train, y_train, np.array([[1.]]), iters=iterations,
